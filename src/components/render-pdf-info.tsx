...
export function RenderPdfInfo({ data }: RenderPdfInfoProps) {
  ...
  const totalEgresos = egresos.reduce((acc, m) => acc + parseMonto(m.monto), 0);
  ...
  const saveEdit = () => {
    ...
    editing.tipo === 'ingreso' ? setIngresos(update) : setEgresos(update);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border bg-card p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            <span>
              Inicial:{' '}
              <strong className="text-foreground">{data.saldo_inicial}</strong>
            </span>
            <span>
              Final:{' '}
              <strong className="text-foreground">{data.saldo_final}</strong>
            </span>
          </div>
        </div>
      ...
      <Collapsible open={openIngresos} onOpenChange={setOpenIngresos}>
        ...
        <CollapsibleContent
          forceMount
          className="mt-2 rounded-xl border bg-muted/20 px-4 py-3 data-[state=closed]:hidden overflow-x-auto"
        >
          <MovementsTable
            title="Ingresos"
            data={ingresos}
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {showTipoGasto && <TableHead>Tipo de gasto</TableHead>}
            ...
            <TableRow key={i}>
              ...
              {showTipoGasto && (
                <TableCell>
                  <Select
                    value={m.tipoGasto}
                    onValueChange={(v) => onTipoGastoChange?.(i, v)}
                  >
