...
/** Hueco del tamaño de la card mientras llega la primera respuesta. */
function CardEsqueleto() {
  return (
    <div className="rounded-[12px] border border-[var(--arca-border)] bg-[var(--arca-surface)] p-4">
      <div className="flex items-center gap-2">
        <Scale className="size-4 text-[var(--arca-ink-4)]" strokeWidth={2} />
        <span className="text-[13px] font-semibold text-[var(--arca-ink-3)]">
          Banco vs Facturación
        </span>
        <Loader2 className="ml-auto size-3.5 animate-spin text-[var(--arca-ink-4)]" />
      </div>
      <div className="mt-3 grid grid-cols-3 gap-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="flex flex-col gap-1.5">
            <div className="h-[11px] w-20 animate-pulse rounded bg-[var(--arca-surface-2)]" />
            <div className="h-[19px] w-28 animate-pulse rounded bg-[var(--arca-surface-2)]" />
          </div>
        ))}
      </div>
      <div className="mt-3 h-[38px] animate-pulse rounded-[10px] bg-[var(--arca-surface-2)]" />
    </div>
  );
}
...
export function BancoVsFacturacionCard({
  ...
  const { data, isFetching } = useQuery({
    ...
    enabled: !!clienteId,
    // Cambiar de mes mantiene a la vista los números del mes anterior
    // (atenuados) en vez de vaciar la card: desaparecer y volver se lee como
    // que algo se rompió.
  ...
  // una card que después se esconde.
  if (!data) return compacto ? null : <CardEsqueleto />;
  ...
  return (
    <div className="rounded-[12px] border border-[var(--arca-border)] bg-[var(--arca-surface)] p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* Los números de abajo son del mes anterior hasta que llegue el
            nuevo: hay que decirlo, no solo atenuarlos. */}
        {isFetching && (
        ...
        {sinDatos ? (
          ...
          <>
            ...
            <p className="mt-2 text-[11px] text-[var(--arca-ink-4)]">
              ...
              {data.comprobantes} comprobante
              {data.comprobantes !== 1 ? 's' : ''}
