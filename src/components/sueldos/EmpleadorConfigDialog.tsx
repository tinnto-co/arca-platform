...
function CatalogSelect({
  ...
  return (
    ...
    >
      ...
      <SelectContent>
        <SelectItem value={NONE} className="text-[13px] text-[var(--arca-ink-3)]">
          — Sin configurar —
        </SelectItem>
        {options.map((o) => (
...
export function EmpleadorConfigDialog({ open, onOpenChange, clientId, empresaNombre }: Props) {
  ...
  const sectionCls = 'text-[11px] font-semibold uppercase tracking-wider text-[var(--arca-ink-3)] mb-2';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-[15px]">Configuración de empleador</DialogTitle>
          <p className="text-[13px] text-[var(--arca-ink-2)]">{empresaNombre}</p>
        </DialogHeader>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={NONE} className="text-[13px] text-[var(--arca-ink-3)]">— Sin configurar —</SelectItem>
                        <SelectItem value="C" className="text-[13px]">C — Por CUIL</SelectItem>
                        <SelectItem value="L" className="text-[13px]">L — Por Legajo</SelectItem>
                        <SelectItem value="N" className="text-[13px]">N — Por Nombre</SelectItem>
                      </SelectContent>
                    </Select>
                  )}
                />
                ...
                render={({ field }) => (
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      className="scale-90"
                    />
                    <Label htmlFor="seguro" className="text-[13px] cursor-pointer">
                ...
                render={({ field }) => (
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      className="scale-90"
                    />
                    <Label htmlFor="mipyme" className="text-[13px] cursor-pointer">
                      Certificado MiPyME
                    </Label>
                  </div>
                )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className={fieldCls}>
                ...
                <Controller name="situacionDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={situaciones} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls}>
                <Label className={labelCls}>Condición del trabajador</Label>
                <Controller name="condicionDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={condiciones} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls}>
                <Label className={labelCls}>Actividad SIJP</Label>
                <Controller name="actividadDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={actividades} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls}>
                <Label className={labelCls}>Modalidad de contratación</Label>
                <Controller name="contratacionDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={modalidades} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls}>
                <Label className={labelCls}>Tipo de siniestro</Label>
                <Controller name="siniestradoDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={siniestrados} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls}>
                <Label className={labelCls}>Zona geográfica</Label>
                <Controller name="zonaDefaultId" control={control}
                  render={({ field }) => (
                    <CatalogSelect value={field.value} onChange={field.onChange}
                      options={zonas} placeholder="Seleccioná..." />
                  )} />
              </div>

              <div className={fieldCls + ' col-span-2'}>
                <Label className={labelCls}>Obra social</Label>
                <Controller name="obraSocialDefaultId" control={control}
                  render={({ field }) => (
                    <Select
                      value={field.value ?? NONE}
                      onValueChange={(v) => field.onChange(v === NONE ? null : v)}
