/**
 * Seed payroll_localidad with AFIP LSD localidad codes.
...
async function main() {
  ...
  console.log(`Seeding payroll_localidad con ${LOCALIDADES.length} registros...\n`);

  let inserted = 0;
  let skipped = 0;

  for (const { codigo, nombre } of LOCALIDADES) {
    const existing = await sql`SELECT id FROM payroll_localidad WHERE codigo = ${codigo}`;
    if (existing.length > 0) {
      skipped++;
      continue;
    }
    await sql`INSERT INTO payroll_localidad (id, codigo, nombre) VALUES (gen_random_uuid(), ${codigo}, ${nombre})`;
    inserted++;
  }

  const total = await sql`SELECT COUNT(*) as total FROM payroll_localidad`;
