...
async function bulkImportAndNotify(excelPath: string) {
  ...
  const [ownerUser] = await db.select().from(user).where(eq(user.id, OWNER_USER_ID)).limit(1);
  if (!ownerUser) {
    console.error(
  ...
  for (const sheetName of sheetNames) {
    ...
    const matrix = (XLSX.utils.sheet_to_json as (s: typeof sheet, opts?: { header?: number }) => unknown[])(
      sheet,
      { header: 1 }
    ) as unknown[][];
    ...
    if (!headerRow || headerRow.length <= firstClientCol) {
      console.warn(`⚠️ Hoja "${sheetName}": no hay columnas de clientes, omitiendo.`);
      continue;
    }
    ...
    for (let r = 0; r < matrix.length; r++) {
      const label = normLabel((matrix[r] as unknown[])?.[labelCol]);
    ...
    if (rowCuit < 0 || rowClaveFiscal < 0) {
      console.warn(`⚠️ Hoja "${sheetName}": no se encontraron filas 'Cuit' y 'Clave Fiscal', omitiendo.`);
      continue;
    }

    const getCell = (row: number, col: number): string =>
      String((matrix[row] as unknown[])?.[col] ?? "").trim();

    const clients: { nombre: string; cuit: string; relacionClave: string; claveFiscal: string }[] = [];
    for (let c = firstClientCol; c < headerRow.length; c++) {
      ...
      const relacionClave = rowRelacionClave >= 0 ? getCell(rowRelacionClave, c) : "";
    ...
    try {
      ...
      const identityNumber = (row.relacionClave && row.relacionClave !== "")
      ...
      const [existing] = await db
        .select()
        .from(client)
        .where(and(eq(client.userId, OWNER_USER_ID), eq(client.identityNumber, identityNumber)))
        .limit(1);
      if (existing) {
        console.log(`   ⏭️ Ya existe en la DB (ID: ${existing.id}), notificando para scrapear...`);
        await axios.post(`${BACKEND_URL}/api/scrap/new-client`, {
          clientId: existing.id,
        });
        console.log(`   ✔️ Notificación de scrape enviada. Esperando ${SCRAPE_DELAY_MS / 1000}s antes del siguiente...`);
      ...
      const detail = err?.cause
        ? String((err.cause as { message?: string; code?: string })?.message ?? err.cause)
        : err?.detail ?? err?.message ?? String(error);
      console.error(`   ❌ Error con cliente ${row.nombre}:`, detail);
      if (err?.code === "23503") {
        console.error("   💡 Posible FK: verifica que el usuario OWNER_USER_ID exista en la tabla user.");
      }
      continue;
    }
  }
